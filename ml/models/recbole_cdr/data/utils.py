...
def get_dataloader(config, phase, domain='target'):
    ...
    if phase == 'train':
        ...
        # recbole>=1.2 uses valid_neg_sample_args/test_neg_sample_args instead of eval_neg_sample_args
        eval_neg = config['valid_neg_sample_args'] or config['eval_neg_sample_args'] or {}
        if eval_neg.get('sample_num', 'none') == 'none':
...
def create_source_samplers(config, dataset, built_datasets):
    ...
    eval_neg_sample_args = config['valid_neg_sample_args'] or config['eval_neg_sample_args'] or {}
