...
class OverlapDataloader(AbstractDataLoader):
    ...
    def _init_batch_size_and_step(self):
        ...
        self.sample_size = len(self._dataset)
    ...
    def collate_fn(self, batch):
        """Index into dataset using the batch of indices provided by PyTorch's DataLoader."""
        return self._dataset[batch]
...
class CrossDomainDataloader(AbstractDataLoader):
    ...
    def _init_batch_size_and_step(self):
        ...
        self.step = batch_size
        self._batch_size = batch_size
        self.sample_size = self.target_dataloader.sample_size

    def reinit_pr_after_map(self):
        pass  # no-op: pr tracking removed in recbole 1.2.1
    ...
    def __iter__(self):
        if self.state == CrossDomainDataLoaderState.SOURCE:
            return iter(self.source_dataloader)
        elif self.state == CrossDomainDataLoaderState.TARGET:
            return iter(self.target_dataloader)
        elif self.state == CrossDomainDataLoaderState.BOTH:
            self._source_iter = iter(self.source_dataloader)
            self._target_iter = iter(self.target_dataloader)
            return self
        elif self.state == CrossDomainDataLoaderState.OVERLAP:
            return iter(self.overlap_dataloader)
    ...
    def __next__(self):
        if self.state == CrossDomainDataLoaderState.BOTH:
            try:
                source_data = next(self._source_iter)
            except StopIteration:
                # Restart source when exhausted (target drives epoch length)
                self._source_iter = iter(self.source_dataloader)
                source_data = next(self._source_iter)
            target_data = next(self._target_iter)  # StopIteration propagates to end epoch
...
class CrossDomainFullSortEvalDataLoader(FullSortEvalDataLoader):
    ...
    def __init__(self, config, dataset, source_dataset, sampler, shuffle=False):
        ...
        self._dataset = source_dataset  # recbole base class uses _dataset in _init_batch_size_and_step
