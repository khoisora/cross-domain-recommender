...
This model optimizes RMSE on raw star ratings (1-5). It serves as a baseline
to show that explicit rating prediction is a poor objective for top-K ranking.
"""
...
from .base_recommender import BaseLibraryRecommender
from .id_utils import normalize_id, normalize_maps

logger = logging.getLogger(__name__)


class MatrixFactorizationExplicit(BaseLibraryRecommender):
    ...
    def fit(self, ratings: pd.DataFrame, user_to_idx: dict, item_to_idx: dict,
            epochs: int = 100, lr: float = 0.01, reg_lambda: float = 0.005,
            **kwargs) -> dict[str, float]:
        """Train Surprise SVD on ALL ratings (explicit feedback)."""
        user_to_idx, item_to_idx = normalize_maps(user_to_idx, item_to_idx)

        # Build Surprise training data — use raw star ratings
        rows = []
        for _, r in ratings.iterrows():
            uid = normalize_id(r["user_id"])
            iid = normalize_id(r["item_id"])
            if uid in user_to_idx and iid in item_to_idx:
                rows.append((uid, iid, float(r["rating"])))

        df = pd.DataFrame(rows, columns=["user", "item", "rating"])
        logger.info("Surprise SVD: %d explicit ratings", len(df))
        ...
        # Extract embeddings mapped to our index space
        ...
        for inner_uid in range(trainset.n_users):
            raw = str(trainset.to_raw_uid(inner_uid))
            idx = user_to_idx.get(raw)
            if idx is not None:
                user_emb[idx] = self.model.pu[inner_uid]
                self._user_biases[idx] = self.model.bu[inner_uid]

        for inner_iid in range(trainset.n_items):
            raw = str(trainset.to_raw_iid(inner_iid))
            idx = item_to_idx.get(raw)
            if idx is not None:
    ...
    def predict(self, user_idx: int, item_indices=None) -> np.ndarray:
        ...
        return self._global_mean + b_u + self._item_biases[item_indices] + self.item_embeddings[item_indices] @ u
